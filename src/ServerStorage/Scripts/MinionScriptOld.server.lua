...
function fireOnHumanoid(human)
	...
	if char then
		...
		if hrp then
			local range = distanceTo(hrp.Position)
			local distance = 0
			local speed = 48
			local connection
			connection = game:GetService("RunService").Heartbeat:connect(function(dt)
				range = distanceTo(hrp.Position)
				distance = distance + speed * dt
				
				local vector = (hrp.Position - HRP.Position)
				local direction = vector.unit
				local position = HRP.Position + direction * distance
				sfx.StaticExplosion:Invoke(position, 1, MINION.Torso.BrickColor.Name)
