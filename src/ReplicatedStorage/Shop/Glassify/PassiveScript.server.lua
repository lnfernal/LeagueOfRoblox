...
function script.Passive.OnInvoke(hums,damage)
	...
	local speed = CONTROL.GetStat:Invoke("H4x") * .15
	ST.StatBuff:Invoke(HUMAN,"H4x",speed,3)   
	ST.StatBuff:Invoke(HUMAN,"H4xVampirism",.15,3) 
	local pos = HUMAN.Parent.HumanoidRootPart.Position
	SFX.Shockwave:Invoke(Vector3.new(pos.X,pos.Y - 2.5, pos.Z), 4, "Really blue")
	active = false
	delay(10,function() 
	active = true
	end)
