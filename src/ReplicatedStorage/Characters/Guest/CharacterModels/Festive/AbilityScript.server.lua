local colors = {"Bright red","Lime green","White"}
...
function ability1(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 8, 16, {BrickColor = d.C((colors))}, 0.3)
	wait(0.3)
	d.SFX.Shockwave:Invoke(d.FLAT(d.HRP.Position), 8,colors)
...
wait(0.15)
	...
	local function onHit(enemy)
		d.CONTROL.AbilityCooldownReduce:Invoke("B", 0.3)
			d.CONTROL.AbilityCooldownReduce:Invoke5("D", 0.3)
...
function ability3(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 4, 32, {BrickColor = d.C(colors[math.random(1,#colors)])}, 0.25)
end
...
function ability4(d)
	...
	d.SFX.Bolt:Invoke(d.HRP.Position, 0.4, colors[math.random(1,#colors)], 0.25)
	wait(0.25)
	d.PLAY_SOUND(d.HUMAN, 12222152, nil, 1.25)
	d.SFX.Bolt:Invoke(d.HRP.Position, 0.4, colors[math.random(1,#colors)], 0.35)
	wait(0.35)
	...
	local function onHit(projectile, enemy)
		d.SFX.Artillery:Invoke(d.FLAT(projectile.Position), 4, "Bright red")
	...
	d.SFX.ProjShrink:Invoke(p:ClientArgs(), 3, colors, 0.4)
end
