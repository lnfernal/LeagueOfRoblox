...
function ability1(d)
	...
	wait(0.3)
	d.SFX.Shockwave:Invoke(d.FLAT(d.HRP.Position), 8, "Medium blue")
	
	d.CHAR.Sword.ParticleEmitter.Enabled = false
	d.CHAR.Torso.SpecialParts.Enabled = false
	...
	d.CHAR.Sword.ParticleEmitter.Enabled = true
	d.CHAR.Torso.SpecialParts.Enabled = true	
	d.SFX.Shockwave:Invoke(d.FLAT(d.HRP.Position), 8, "Medium blue")
	d.SFX.Bolt:Invoke(d.FLAT(d.HRP.Position), 1,  "Medium blue")
		d.SFX.Explosion:Invoke(d.FLAT(d.HRP.Position), 8, "Medium blue")
	end)
end
...
wait(0.15)
	...
	d.SFX.Explosion:Invoke(center, radius, "Medium blue")
end
...
function ability3(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 4, 32, {BrickColor = d.C("Medium blue")}, 0.25)
	for i,parts in pairs(d.CHAR:GetChildren()) do
		if parts.Name == "Outline" and parts:IsA("Part") then
...
function ability4(d)
	...
	d.SFX.Bolt:Invoke(d.HRP.Position, 0.4, "Medium blue", 0.25)
	wait(0.25)
	d.PLAY_SOUND(d.HUMAN, 12222152, nil, 1.25)
	d.SFX.Bolt:Invoke(d.HRP.Position, 0.4, "Medium blue", 0.35)
	wait(0.35)
	...
	local function onHit(projectile, enemy)
		d.SFX.Artillery:Invoke(d.FLAT(projectile.Position), 4, "Medium blue")
	...
	d.SFX.ProjShrink:Invoke(p:ClientArgs(), 3, "Medium blue", 0.4)
end
...
local abilityData = {
	...
	B = {
		Name = "Safe Chat",
		Desc = "The guest says a safe chat, bothering nearby enemies to deal <damage> h4x damage, and slow them by <percent>% for 2 seconds. Each enemy hit also reduces this Safe Chat and Report Abuse's cooldown by 0.3 seconds, because the guest, of course, gains energy from annoying many people.",
		MaxLevel = 5,
		damage = {
			Base = 20,
			AbilityLevel = 20,
