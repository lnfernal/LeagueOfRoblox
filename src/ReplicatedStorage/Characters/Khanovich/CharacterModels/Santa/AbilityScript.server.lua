...
mesh.TextureId = "http://www.roblox.com/asset/?id=140876987"
...
function basicAttack(d)
	...
	d.PLAY_ANIM(d.PLAYER, "https://www.roblox.com/item.aspx?id=522991232")
...
function ability1(d)
	...
	d.CONTROL.AbilityCooldown:Invoke("A", 10 - (10 * d.CONTROL.GetStat:Invoke("CooldownReduction")))
		d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 8, 16, {BrickColor = d.C("Bright red")}, 0.15)
	local damage = ability:C(data.damage)
	local slow = -ability:C(data.slow)/100
	local duration = ability:C(data.duration)
	
	d.PLAY_ANIM(d.PLAYER, "https://www.roblox.com/item.aspx?id=522991916")
	wait(0.3)
...
function ability2(d)
	...
	local function onHit(p, enemy)
		...
		if hrp then
			d.DS.Damage:Invoke(enemy, damage, "Toughness", d.PLAYER)
			d.SFX.Artillery:Invoke(d.FLAT(hrp.Position), 8, "Bright red")
...
function ability3(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 8, 32, {BrickColor = d.C("White"), Transparency = 0.5}, 0.5)
	wait(0.625)
...
local abilityData = {
	A = {
		Name = "Candy Cane Throw",
		Desc = "Khanovich throws his candy cane, dealing <damage> damage to each enemy it passes through. It also slows them <slow>% for <duration> seconds.",
		MaxLevel = 5,
		damage = {
			Base = 20,
			AbilityLevel = 8,
	...
	C = {
		Name = "HO HO HO!",
		Desc = "Khanovich lets out a jingle, dealing <damage> damage and stunning nearby enemies for anywhere from 1 to 1.5 seconds. The radius of this ability is <radius> studs.",
		MaxLevel = 5,
		radius = {
			Base = 13,
			AbilityLevel = 0.75
