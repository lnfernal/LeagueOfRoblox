...
function ability1(d)
	...
	local function onEnd(p)
		if hitAnEnemy then 
			...
			local function onHit(enemy)
				...
				d.ST.DOT:Invoke(enemy, (enemy.MaxHealth * .05) + damage, 3, "Toughness", d.PLAYER, "rest in pieces life")
			end
	...
	d.SFX.ProjMeander:Invoke(p:ClientArgs(), 1, ("Teal"), 0.2)  
end
...
function ability2(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 4, 12, {BrickColor = d.C("Teal")}, 0.35)
	wait(0.5)
	...
	d.SFX.ProjMeander:Invoke(args, 2, ("Teal"), 0.2)
	repeat
	wait(.1)
	circles = circles + 1
	d.SFX.Circles:Invoke(topos, 6,"Teal",.2,dir)
	until circles == 4
end
...
function ability3(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 4, 24, {BrickColor = d.C("Teal")}, 0.35)
	wait(0.5)
...
function ability4(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 12, 48, {BrickColor = d.C("Teal")}, 0.2)
