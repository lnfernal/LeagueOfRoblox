function basicAttack(d)
	...
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,"Medium blue", 0.25,direction,"DiamondPlate",0.039) 
end
...
function ability1(d)
	...
	d.SFX.Shockwave:Invoke(b, radius, "Medium blue", 0.65)
	wait(0.65)
	local team = d.CHAR.Team.Value
	...
	d.PLAY_SOUND_POS(b, 84937942)
	d.SFX.Artillery:Invoke(b, radius, "Medium blue")
	d.SFX.Artillery:Invoke(d.FOOT(), radius, "Medium blue")
end
...
function ability3(d)
	...
	d.SFX.Bolt:Invoke(d.HRP.Position, 0.5, "Medium blue", 0.35)
	wait(0.425)
	d.PLAY_SOUND(d.HUMAN, 84937942)
	...
	d.SFX.Artillery:Invoke(center, radius, "Medium blue")
end
...
function ability4(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 24, 48, {BrickColor = d.C("Medium blue")}, 1)
	wait(1)

	local t = 0
	local radius = 24
	d.SFX.AreaAOEFollow:Invoke(d.HRP, radius, "Medium blue",duration)
	while t < duration do
		local dt = wait(0.5)
		t = t + dt
		...
		local function onHit(enemy)
			...
			d.SFX.PartRandomFollow:Invoke(enemy.Parent.Torso, 1.5, "Medium blue")
		end
		d.DS.AOE:Invoke(center, radius, team, onHit)
...
local abilityData = {
	...
	C = {
		Name = "Deep Freeze",
		Desc = "FlareBlast unleashes a frost wave, stunning nearby opponents for <duration> seconds while dealing <damage> damage to them. Frost which freezes enemies returns to FlareBlast, toughening his armor which increases his Toughness and Resistance <buff> for <buffDuration> seconds.",
		MaxLevel = 5,
		duration = {
			Base = 1,
			AbilityLevel = 0.1,
