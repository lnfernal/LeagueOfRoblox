...
local shot = Instance.new("Part")
	...
	shot.BrickColor = BrickColor.new("Bright purple") 
	...
	particleem.Texture = "rbxassetid://272050333"
	local colors = {ColorSequenceKeypoint.new(0, Color3.fromRGB(170,0,255))
		,ColorSequenceKeypoint.new(0.209, Color3.fromRGB(71,0,107))
		,ColorSequenceKeypoint.new(0.524, Color3.fromRGB(161,0,172))
		,ColorSequenceKeypoint.new(0.727, Color3.fromRGB(221,121,220))
		,ColorSequenceKeypoint.new(1, Color3.fromRGB(224,131,255))}
...
function basicAttack(d)
	...
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,"Bright purple", 0.25,direction,"Neon",0.039) 
end
...
function ability1(d)
	...
	local direction = d.HRP.CFrame.lookVector
	local part = shot:Clone()
	local particle = particleem:Clone()
	particle.Parent = part
	particle.LightEmission = 0.960
	part.Parent = game.ReplicatedStorage
	d.DB(part)
	local function onEnd(projectile)
		part:Destroy()
...
function ability2(d)
	...
	d.SFX.Explosion:Invoke(center, 10,"Bright purple", 0.25)
    wait(0.35)
	local position = d.CHAR["Right Arm"].Position
	local direction = d.HRP.CFrame.lookVector
	local speed = 48
...
effect.BrickColor = BrickColor.new("Bright purple")
			...
			delay(2, function()
				...
				d.SFX.Explosion:Invoke(pos, 15, "Bright purple")
				end
...
function ability3(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 6,"Bright purple", 0.3)
    wait(0.4)
	...
	part.ParticleEmitter.Color = ColorSequence.new(colors)
	part.Parent = game.ReplicatedStorage
	d.DB(part)
...
function ability4(d)
	...
	if hit == false then
		...
		local delta = Vector3.new(0, 0, -2)
			...
			Usage = Usage + 1
			d.SFX.Bolt:Invoke(ally.Parent:FindFirstChild("HumanoidRootPart").Position, 1,"Bright purple", 0.35)
			d.SFX.Explosion:Invoke(ally.Parent:FindFirstChild("HumanoidRootPart").Position, 8, "Bright purple")
			hit = false
			wait(4)
		hit = true	
		end
