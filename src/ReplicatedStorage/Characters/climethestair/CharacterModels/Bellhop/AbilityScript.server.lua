...
function basicAttack(d)
	...
	local part = game.ReplicatedStorage.Items.ClimeBasic:Clone()
...
function ability1(d)
	...
	local position = d.CHAR["Right Arm"].Position
	local direction = d.HRP.CFrame.lookVector
	local part = game.ReplicatedStorage.Items.Rapid:Clone()
	part.Parent = game.ReplicatedStorage
	d.DB(part)
	local function onEnd(projectile)
		part:Destroy()
...
function ability2(d)
	...
	local part = game.ReplicatedStorage.Items.Clime2nd:Clone()
	part.Parent = game.ReplicatedStorage
	d.DB(part)
...
function ability3(d)
	...
	local part = game.ReplicatedStorage.Items.DragonsHead:Clone()
	part.Parent = game.ReplicatedStorage
	d.DB(part)
...
function ability4(d)
	...
	local teleport = game.ServerStorage.Elevator:clone()
	...
	if hit == false then
		...
		local delta = Vector3.new(0, 0, -2)
			...
			Usage = Usage + 1
			d.SFX.Bolt:Invoke(ally.Parent:FindFirstChild("HumanoidRootPart").Position, 1,"Bright red", 0.35)
			d.SFX.Explosion:Invoke(ally.Parent:FindFirstChild("HumanoidRootPart").Position, 8, "Bright red")
			hit = false
			wait(4)
		hit = true	
		end
...
local abilityData = {
	...
	D = {
		Name = "Abyssal Elavator",
		Desc = "climethestair builds a elavator in a specified location, the elavator will teleport any ally who touches it to him. and has a duration of 3 minutes",
		MaxLevel = 1,
	}
}
