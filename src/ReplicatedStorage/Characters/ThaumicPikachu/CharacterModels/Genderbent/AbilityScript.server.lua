...
function ability1(d)
	...
	local function onHit(projectile, enemy)
		...
		d.SFX.Cloud:Invoke(center, radius, "Carnation pink", duration)
		end
	end
	local function onStep(projectile)
	end
	local function onEnd(projectile)
		...
		created = true
		d.DS.AOE:Invoke(center, radius, team, onHitt)
		d.SFX.Cloud:Invoke(center, radius, "Carnation pink")
		end
	end
...
function ability2(d)
	...
	local function onHit(projectile, enemy)
		...
		d.ST.StatBuff:Invoke(enemy, "Resistance", shred, duration)
		d.DS.AOE:Invoke(center, radius, team, onHitt)
		d.SFX.Explosion:Invoke(center, radius, "Lilac")
		end
	end
	local function onStep(projectile)
	end
...
local abilityData = {
	A = {
		Name = "Love Potion",
		Desc = "ThaumicPikachu throws a infatuating potion which explodes upon reaching it's maximum range, it creates a love field which slows by <slow>% for <duration> seconds dealing <damage> over <duration> seconds.",
		MaxLevel = 5,
		damage = {
			Base = 15,
			AbilityLevel = 10,
	...
	B = {
		Name = "Corrosion Perfume",
		Desc = "ThaumicPikachu throws a perfume vial that eats away at the target's and nearby enemies defenses by <shred>%, it also deals 5% of their maximum health as true damage. This lasts <duration> seconds.",
		MaxLevel = 5,
		shred = {
			Base = 20,
			AbilityLevel = 3,
