...
Part.Anchored = true
local blumesh = Instance.new("SpecialMesh")
blumesh.MeshId = "http://www.roblox.com/asset/?id=225891571"
blumesh.Scale = Vector3.new(0.441, 0.441, 0.441)
blumesh.TextureId = "http://www.roblox.com/asset/?id=225891637"
local redmesh = Instance.new("SpecialMesh")
redmesh.MeshId = "http://www.roblox.com/asset/?id=226206743"
redmesh.Scale = Vector3.new(0.441, 0.441, 0.441)
redmesh.TextureId = "http://www.roblox.com/asset/?id=226207214"
local gmesh = Instance.new("SpecialMesh")
gmesh.MeshId = "http://www.roblox.com/asset/?id=225895202"
gmesh.Scale = Vector3.new(0.441, 0.441, 0.441)
gmesh.TextureId = "http://www.roblox.com/asset/?id=225895244"
local pmesh = Instance.new("SpecialMesh")
pmesh.MeshId = "http://www.roblox.com/asset/?id=110685499"
pmesh.Scale = Vector3.new(0.441, 0.441, 0.441)
pmesh.TextureId = "http://www.roblox.com/asset/?id=110685557"
local cmesh = Instance.new("SpecialMesh")
cmesh.MeshId = "http://www.roblox.com/asset/?id=225889848"
cmesh.Scale = Vector3.new(0.441, 0.441, 0.441)
cmesh.TextureId = "http://www.roblox.com/asset/?id=225889967"
local function Pick()
...
function basicAttack(d)
	...
	if choice == "Regular" then
		blumesh:Clone().Parent = part
	elseif choice == "Rage" then
		redmesh:Clone().Parent = part
	else --Poison
		gmesh:Clone().Parent = part		
	end
...
function ability1(d)
	...
	local function onHitt(enemy)
		d.ST.DOT:Invoke(enemy, damage, duration, "Resistance", d.PLAYER, "Poisoned!")
	...
	local function onHit(projectile, enemy)
		...
		d.SFX.Cloud:Invoke(center, radius, "Bright red", duration)
		end
	end
	local function onStep(projectile)
	end
	local function onEnd(projectile)
		...
		created = true
		d.DS.AOE:Invoke(center, radius, team, onHitt)
		d.SFX.Cloud:Invoke(center, radius, "Bright red")
		end
	end
...
function ability2(d)
	...
	local function onHit(projectile, enemy)
		...
		d.ST.StatBuff:Invoke(enemy, "Resistance", shred, duration)
		d.DS.AOE:Invoke(center, radius, team, onHitt)
		d.SFX.Explosion:Invoke(center, radius, "Bright yellow")
		end
	end
	local function onStep(projectile)
	end
...
function ability3(d)
	...
	local function onEnd(projectile)
		...
		d.SFX.Explosion:Invoke(center, radius, "Bright green")
	end
	local p = d.DS.AddProjectile:Invoke(position, direction, speed, width, range, team, onHit, onStep, onEnd, nil, true)
end
...
local abilityData = {
	...
	D = {
		Name = "Mad Chemist",
		Desc = "ThaumicPikachu consumes an unkown vial which grants him <buff>% to all stats, after <duration> seconds, he starts to feel side-effects reducing his stats by <debuff>% for half the duration. After which he starts to toss random potions every 1 second for <duration> seconds from poison(<poisondamage> damage for 2.5 seconds and <slow>% for 1.5 seconds) ,debuff (<debuff>% of resistance and toughness),bonus damage (<damage> damage)",
		MaxLevel = 3,
		buff = {
			Base = 30,
		},
