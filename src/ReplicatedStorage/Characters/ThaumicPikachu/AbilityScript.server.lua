...
function basicAttack(d)
	...
	elseif choice == "Rage" then
		...
		part.Mesh.VertexColor = Vector3.new(0.25, 0.8, 0.25)		
	end
...
function ability1(d)
	...
	local function onHit(projectile, enemy)
		...
		d.SFX.Cloud:Invoke(center, radius, "Forest green", duration)
		end
	end
	local function onStep(projectile)
	end
	local function onEnd(projectile)
		...
		created = true
		d.DS.AOE:Invoke(center, radius, team, onHitt)
		d.SFX.Cloud:Invoke(center, radius, "Forest green")
		end
	end

	
	local p = d.DS.AddProjectile:Invoke(position, direction, speed, width, range, team, onHit, onStep, onEnd, nil, true)
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,"Forest green", 0.25,direction,"Neon",0.039)
	d.SFX.ProjPart:Invoke(p:ClientArgs(), part, CFrame.Angles(math.pi/2, 0, 0), {Spin = CFrame.Angles(0, 1/3, 0)})
...
function ability2(d)
	...
	part.Parent = game.ReplicatedStorage
	d.DB(part)
	part.Mesh.VertexColor = Vector3.new(0.2, 0.2, 0.2)
	...
	local function onHit(projectile, enemy)
		...
		d.ST.StatBuff:Invoke(enemy, "Resistance", shred, duration)
		d.DS.AOE:Invoke(center, radius, team, onHitt)
		d.SFX.Explosion:Invoke(center, radius, "Dark stone grey")
		end
	end
	local function onStep(projectile)
	end
	...
	local p = d.DS.AddProjectile:Invoke(position, direction, speed, width, range, team, onHit, onStep, onEnd, nil, true)
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,"Dark stone grey", 0.25,direction,"Neon",0.039)
	d.SFX.ProjPart:Invoke(p:ClientArgs(), part, CFrame.Angles(math.pi/2, 0, 0), {Spin = CFrame.Angles(0, 1/3, 0)})
end
...
function ability3(d)
	...
	part.Parent = game.ReplicatedStorage
	d.DB(part)
	part.Mesh.VertexColor = Vector3.new(0.5, 0.5, 0.85)
...
local abilityData = {
	...
	B = {
		Name = "Corrosion Vial",
		Desc = "ThaumicPikachu throws a vial that eats away at the target's and nearby enemies defenses by <shred>%, it also deals 5% of their maximum health as true damage. This lasts <duration> seconds.",
		MaxLevel = 5,
		shred = {
			Base = 20,
			AbilityLevel = 3,
