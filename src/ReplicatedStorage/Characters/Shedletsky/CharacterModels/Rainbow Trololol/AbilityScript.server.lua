...
function basicAttack(d)
	...
	elseif attackNumber == 3 then
		...
		d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,BrickColor.palette(math.random(0, 63)).Color, 0.25,direction,"Neon",0.039)
...
function ability1(d)
	...
	d.SFX.Shockwave:Invoke(center, range, BrickColor.palette(math.random(0, 63)).Color)
end
...
function ability2(d)
	...
	d.SFX.Line:Invoke(d.HRP.Position,d.HRP.CFrame.lookVector, range, width, BrickColor.palette(math.random(0, 63)).Color)
end
...
function ability3(d)
	...
	wait(.1)
	circles = circles + 1
	d.SFX.Circles:Invoke(topos, 6,BrickColor.palette(math.random(0, 63)).Color,.2,dir)
	until circles == 3
end
...
function ability4(d)
	...
	local arena = nil
	if script.Parent.Parent.Character.RequiredLevel.Value == 20
	then
	...
	d.DS.ConstructArena:Invoke(arena, position, constructionTime, duration,"random")
end
