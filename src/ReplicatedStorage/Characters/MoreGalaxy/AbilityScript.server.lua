...
function ability3(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 6,"Lime green", 0.35)
    wait(0.45)
	local tagName = d.PLAYER.Name.."Virus!"
	local position = d.HRP.Position
	local speed = 60
...
local p = d.DS.AddProjectile:Invoke(position, direction, speed, width, range, team, onHit, onStep, onEnd, nil, false)
	...
	d.SFX.ProjZap:Invoke(p:ClientArgs(), 1, "Lime green", 0.1)
end
...
function ability4(d)
	...
	d.SFX.Trail:Invoke(d.CHAR["Left Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C("Lime green")}, 0.5, 0.4)
		d.SFX.Trail:Invoke(d.CHAR["Right Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C("Lime green")}, 0.5, 0.4)
	...
	d.SFX.Artillery:Invoke(center, 6,"Lime green", 1.5)
	wait(1.5)
	d.DS.AOE:Invoke(center, radius, team, onHit)
	d.SFX.Artillery:Invoke(center, radius, "Lime green")
end
