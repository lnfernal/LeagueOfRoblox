local colors = {"Bright red","Lime green","Lily white"}
...
function ability2(d)
	...
	d.EasyStack(enemy, tagName, 12)
		...
		d.EasyStack(enemy, tagName, 12)
				...
				d.SFX.Explosion:Invoke(position, radius, colors[math.random(1,#colors)])
			end)
		end
	end
		local function onStep(p, dt)
...
function ability4(d)
	...
	d.SFX.Trail:Invoke(d.CHAR["Left Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C(colors[math.random(1,#colors)])}, 0.5, 0.4)
		d.SFX.Trail:Invoke(d.CHAR["Right Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C(colors[math.random(1,#colors)])}, 0.5, 0.4)
