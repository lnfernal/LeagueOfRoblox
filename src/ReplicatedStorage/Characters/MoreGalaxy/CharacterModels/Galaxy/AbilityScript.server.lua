...
function ability2(d)
	...
	d.EasyStack(enemy, tagName, 12)
		...
		d.EasyStack(enemy, tagName, 12)
				...
				d.SFX.Explosion:Invoke(position, radius, "Lapis")
			end)
		end
	end
		local function onStep(p, dt)
...
function ability3(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 6,"Lapis", 0.35)
    wait(0.45)
	local position = d.HRP.Position
	local speed = 60
	local width = 4
...
d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,"Lapis", 0.25,direction,"Neon",0.039)
	d.SFX.ProjZap:Invoke(p:ClientArgs(), 1, "Lapis", 0.1)
end
...
function ability4(d)
	...
	d.SFX.Trail:Invoke(d.CHAR["Left Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C("Lapis")}, 0.5, 0.4)
		d.SFX.Trail:Invoke(d.CHAR["Right Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C("Lapis")}, 0.5, 0.4)
	...
	d.SFX.Artillery:Invoke(center, 6,"Lapis", 1.5)
	wait(1.5)
	d.DS.AOE:Invoke(center, radius, team, onHit)
	d.SFX.Artillery:Invoke(center, radius, "Lapis")
end
