...
mesh.TextureId = "http://www.roblox.com/asset/?id=1158415"
...
function ability2(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 6, "Bright green", 0.4)
	wait(0.45)
	local damage = ability:C(data.damage)
...
function ability3(d)
	...
	d.SFX.Shockwave:Invoke(center, 6, "Bright green", 0.3)
	wait(0.3)
	local function helper(moon)
		...
		d.SFX.Explosion:Invoke(position, radius, "Bright green")
...
local abilityData = {
	A = {
		Name = "LOL Pumpkin",
		Desc = "speedydude900 deploys a LOL Pumpkin to the targeted location. As it travels, it deals <damage> damage to targets nearby. The LOL pumpkin stays there for <duration> seconds, and can be used by speedydude900's other abilities.",
		MaxLevel = 5,
		damage = {
			Base = 10,
			AbilityLevel = 6,
	...
	B = {
		Name = "LOL Gravity",
		Desc = "speedydude900 manipulates gravity around his last Pumpkin created, allowing create another moon from it and move it <range> studs, dealing <damage> and slowing all hit by <slow>% for 2.5 seconds.",
		MaxLevel = 5,
		range = {
			Base = 10,
			AbilityLevel = 5,
	...
	C = {
		Name = "Core Shatter",
		Desc = "speedydude900 shatters his LOL Pumpkins for the LOLs. Enemies nearby take <damage> damage but the moons are destroyed.",
		MaxLevel = 5,
		damage = {
			Base = 7.5,
			AbilityLevel = 7.5,
