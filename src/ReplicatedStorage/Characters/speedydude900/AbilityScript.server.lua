...
function ability2(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 6, "Dark stone grey", 0.4)
	wait(0.45)
...
function ability3(d)
	...
	local center = d.HRP.Position
	local damage = ability:C(data.damage)
	d.SFX.Shockwave:Invoke(center, 6, "Dark stone grey", 0.3)
	wait(0.3)
	local function helper(moon)
		...
		d.SFX.Explosion:Invoke(position, radius, "Dark stone grey")
			...
			if team then
				if team.Value ~= myTeam then
					...
					if enemy then
						...
						while t < 2.5 do
							t = t + wait(0.25)
							d.SFX.Artillery:Invoke(hrp.Position, 1, "Dark stone grey")
						end
						d.PLAY_SOUND(enemy, 84937942)
						d.SFX.Artillery:Invoke(hrp.Position, 6, "Medium grey")
...
local abilityData = {
	...
	B = {
		Name = "LOL Gravity",
		Desc = "speedydude900 manipulates gravity around his last moon created, allowing create another moon from it and move it <range> studs, dealing <damage> slowing all hit by <slow>% for 2.5 seconds.",
		MaxLevel = 5,
		range = {
			Base = 10,
			AbilityLevel = 5,
