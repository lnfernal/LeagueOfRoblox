...
function ability1(d)
	...
	local function onHit(enemy)
		...
		if hrp then
			...
			local function onHit(projectile, enemy)
				...
				d.SFX.Explosion:Invoke(center, 4, "Bright red")
			end
	...
	d.SFX.Shockwave:Invoke(d.FLAT(center), radius, "Bright red",0.2,"Neon")
end
...
function ability2(d)
	...
	d.CHAR["SSJ Particle"].SSJParticle.Enabled = true
	delay(1,function()
		d.CHAR["SSJ Particle"].SSJParticle.Enabled = false
	end)
	...
	for t = 0.01, 0.15, 0.01 do
		d.SFX.ReverseExplosion:Invoke(center, 12, "Bright red", t)
		d.SFX.Explosion:Invoke(center, 12, "Really red", 0.2)  
		end
end
...
function ability3(d)
	...
	local function onHit(projectile, enemy)
		...
		if hrp then
			d.SFX.Shockwave:Invoke(hrp.Position, 4, "Bright red",0.2,"Neon")
		end
	end
	local function onStep(projectile)
	...
	wait(.1)
	circles = circles + 1
	d.SFX.Circles:Invoke(topos, 6,"Bright red",.2,dir, "Neon")
	until circles == 4
end
...
function ability4(d)
	...
	wait(1.3)
	d.PLAY_SOUND(d.HUMAN, 115992246)
