local shot = Instance.new("Part")
	local mesh = Instance.new("SpecialMesh")
		...
		mesh.Parent = shot
local colors = {"Bright red","Bright violet", "Plum", "Black", "White"}
function basicAttack(d)
	...
	local function onHit(p, enemy)	
		...
		local function AOEHit(enemies)
			...
			d.ST.Tag:Invoke(enemies, 5, tagName,true,Color3.new(170,0,255))
		...
		d.SFX.Explosion:Invoke(hrp.Position, 14, "Bright violet")
		end
		end
	end
...
function ability1(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 4, "Bright violet", 0.3)
	wait(.2)
	d.PLAY_ANIM(d.PLAYER, "https://www.roblox.com/PlaceShoot-item?id=583728467")
	d.PLAY_SOUND(d.HUMAN, 99204505)
	wait(.15)
	...
	local bullet = shot:Clone()
	bullet.Parent = game.ReplicatedStorage
	bullet.Mesh.VertexColor = Vector3.new(1, 0, 1)
	d.DB(bullet)
	local position = d.CHAR.GunBarrel.Position
	local tagName = d.PLAYER.Name.."Radiation!"
	local range = 48
	...
	local function onHit(p, enemy)	
		...
		d.ST.Tag:Invoke(enemy, 5, tagName,true,Color3.new(170,0,255))
...
function ability3(d)
	...
	d.SFX.Artillery:Invoke(p.Position, radius,  colors)
	end
...
local abilityData = {
	...
	D = {
		Name = "Cheers!",
		Desc = "After being rescued, PlaceRebuilder will always make sure the guards will never forget him. PlaceRebuilder shines his teeth which fires a beam in a straight line that deals <damage> damage and slows them by <slows>% for 2 seconds. Hitting an enemy with Radiation will heal you <damage> health.",
		MaxLevel = 3,
		damage = {
			Base = 20,
			AbilityLevel = 5,
