...
function basicAttack(d)
	...
	wait(0.1)
	d.PLAY_SOUND(d.HUMAN, 130113322, nil, 0.85)
	
	local position = d.CHAR.Weapon.CFrame:pointToWorldSpace(Vector3.new(0, 0, -2))
	...
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), 0.2,"Lime green", 0.25,direction,"Neon",0.039)
	d.SFX.Explosion:Invoke(position, 1, "Lime green")
...
function ability1(d)
	...
	local center =  d.CHAR.Weapon.CFrame:pointToWorldSpace(Vector3.new(0, 0, -2))
	d.SFX.ReverseExplosion:Invoke(center, 2, "Neon green", 0.3)
	...
	wait(0.15)
	d.PLAY_SOUND(d.HUMAN, 130113322)
	...
	d.SFX.Line:Invoke(position, direction, range, width, "Lime green")
end
...
function ability2(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 8, 16, {BrickColor = d.C("Bright green")}, 0.3)
	local damage = ability:C(data.damage)
	...
	local function onEnd(p)
		delay(0.6, function()
			...
			d.SFX.Explosion:Invoke(position, radius, "Lime green")
	...
	d.SFX.ProjShrink:Invoke(p:ClientArgs(), 1, "Bright green", 0.1,"Neon")
end
...
function ability3(d)
	...
	local center = d.CHAR.Weapon.CFrame:pointToWorldSpace(Vector3.new(0, 0, -2))
	d.SFX.ReverseExplosion:Invoke(center, 10, tostring(d.CHAR.Torso.Skills.Value), 0.45)
	...
	if hrp then
		...
		d.SFX.ProjShrink:Invoke(pz:ClientArgs(), 0.5, "Bright green", 0.25,"Neon")
	end
end
...
function ability4(d)
	...
	local duration = ability:C(data.duration)
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 8, 16, {BrickColor = d.C(tostring("Bright green"))}, 1)
