...
function basicAttack(d)
	...
	local damage = d.CONTROL.GetStat:Invoke("Skillz") * .35
	local hrp = d.HRP
	...
	d.DS.Melee:Invoke(hrp, team, onHit,10,7)	
...
local direction7 = d.HRP.CFrame.lookVector
	...
	local function onHitCombo(enemy)
		...
		d.DS.KnockAirborne:Invoke(enemy, 16, 0.5)
		if brawling == true then
			if brawlEffect == 1 then
...
function ability2(d)
	...
	d.CONTROL.AbilityCooldown:Invoke("B", 8 - (8 * d.CONTROL.GetStat:Invoke("CooldownReduction")))
	record("kick")
	local tocheck = check()
	local function PickRandomEffect()
		local Values = {1, 3}
...
local abilityData = {
	...
	C = {
		...
		stundamage = {
			AbilityLevel = 5,
			Skillz = .3, --this is for the stun kick, the dancing combo also uses this but the 1st uppercut is *0.7 and the 2 punches are 0.4
		},
		duration = {
			Base = .5,
			AbilityLevel = .1,
