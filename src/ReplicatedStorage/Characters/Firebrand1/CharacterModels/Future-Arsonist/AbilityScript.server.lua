...
wait(0.15)
	...
	local function wildfire(center)
		...
		d.DS.AOE:Invoke(center, range, team, onHit)
		d.SFX.Shockwave:Invoke(d.FLAT(center), range, d.CHAR.Torso.Skills.Value.Color,0.2,"Neon")
	end
...
function ability3(d)
	...
	for _, enemy in pairs(enemiesOnFire) do		
		...
		if hrp then
			...
			d.SFX.Artillery:Invoke(hrp.Position, 4, d.CHAR.Torso.Skills.Value.Color,0.2,"Neon")
		end
	end
end
...
function ability4(d)
	...
	local duration = ability:C(data.duration)
	local t = 0
	d.SFX.AreaAOEFollow:Invoke(hrp, radius, d.CHAR.Torso.Skills.Value.Color,duration,"Neon")
	while t < duration do
	...
	d.SFX.PartRandomFollow:Invoke(ally.Parent.Torso, 1.5, d.CHAR.Torso.Skills.Value.Color,0.2,"Neon")
	end
