...
function ability1(d)
	...
	local function onEnd(p)
		...
		d.SFX.Shockwave:Invoke(center, radius, tostring(d.CHAR.Torso.Skills.Value))
	end
	repeat
	...
	d.SFX.ProjMeander:Invoke(p:ClientArgs(), 0.85, tostring(d.CHAR.Torso.Skills.Value), 0.25)
...
function ability2(d)
	...
	local damage = ability:C(data.damage)
	local duration = ability:C(data.duration)
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 4, 24, {BrickColor = d.C(tostring(d.CHAR.Torso.Basicult.Value))}, 0.35)
	...
	local function onEnd(p)
		...
		d.DS.AOE:Invoke(center, radius, team, onHit)
		d.SFX.Shockwave:Invoke(center, radius, tostring(d.CHAR.Torso.Basicult.Value))
	...
	d.SFX.ProjMeander:Invoke(args, 2, tostring(d.CHAR.Torso.Basicult.Value), 0.2)
...
function ability3(d)
	...
	local t = 0
	local radius = 12
	d.SFX.AreaAOEFollow:Invoke(d.CHAR.Torso, radius, tostring(d.CHAR.Torso.Skill3.Value),duration)
	while t < duration do
		...
		local function onHit(enemy)
			...
			d.SFX.PartRandomFollow:Invoke(enemy.Parent.Torso, 3, tostring(d.CHAR.Torso.Skill3.Value))
...
function ability4(d)
	...
	local center = d.HRP.Position
	d.SFX.ReverseExplosion:Invoke(center, 12, tostring(d.CHAR.Torso.Basic.Value), 1.25)
	...
	local p = d.DS.AddProjectile:Invoke(position, direction, speed, width, range, team, onHit, onStep, onEnd)
	d.SFX.ProjShrink:Invoke(p:ClientArgs(), 8, tostring(d.CHAR.Torso.Basic.Value), 1.2)
end
