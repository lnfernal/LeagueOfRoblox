...
sparkles.Color = Color3.new(255,0,0)
...
function basicAttack(d)
	...
	d.SFX.ProjZap:Invoke(p:ClientArgs(), 1, "Maroon", 0.1)
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,"Maroon", 0.25,direction,"Neon",0.039)
end
...
function ability1(d)
	...
	d.SFX.Trail:Invoke(d.CHAR.Staff, Vector3.new(0, 3, 0), 1, {BrickColor = d.C("Maroon")}, 0.5, 0.55)
	...
	d.SFX.Explosion:Invoke(expos,radius,"Maroon")
	end
	end
	SystemStacks(.8)
	end
	...
	wait(.1)
	circles = circles + 1
	d.SFX.Circles:Invoke(topos, 1,"Maroon",.2,dir)
	until circles == 6
end
...
function ability2(d)
	...
	d.SFX.Trail:Invoke(d.CHAR["Left Arm"], Vector3.new(0, -1, 0), 1, {BrickColor = d.C("Maroon")}, 0.5, 0.65)
	...
	d.SFX.Artillery:Invoke(center, 2, "Maroon", 0.75)
	wait(0.75)
	d.DS.AOE:Invoke(center, radius, team, onHit)
	d.SFX.Artillery:Invoke(center, radius, "Maroon")
end
...
local abilityData = {
	A = {
		Name = "Shadow Bolt",
		Desc = "Matt Dusek fires a bolt of pure shadow energy which deals <damage> damage to the first enemy it hits. If this ability is used under Nethermancy, the speed is increased, and this move creates an explosion at the impact zone, spreading the damage to nearby targets.",
		MaxLevel = 6,
		damage = {
			Base = 10,
			AbilityLevel = 10,
	...
	B = {
		Name = "Corruption",
		Desc = "Matt Dusek corrupts the target location which deals <damage> damage to enemies caught within. If this ability is used under Nethermancy, the area is increased slightly and the damage is changed to true damage.",
		MaxLevel = 5,
		damage = {
			Base = 15,
			AbilityLevel = 10,
	...
	C = {
		Name = "Shadowflame",
		Desc = "Matt Dusek fires a piercing bolt of fiery shadow which deals <damage> damage and slowing by 20% for 2.5 seconds to each enemy it passes through. It additionally gains speed and range as it passes through enemies. If this ability is used under Nethermancy, the damage increases by 15% of the current damage per enemy hit while also reducing its cooldown by 1 per enemy hit.",
		MaxLevel = 6,
		damage = {
			Base = 20,
			AbilityLevel = 10,
