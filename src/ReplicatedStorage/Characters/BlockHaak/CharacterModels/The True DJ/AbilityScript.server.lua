...
local TRUEDJ = game.ReplicatedStorage.Remotes.UnlockTrueDJ
...
local controlScript = backpack:WaitForChild("ControlScript")
	...
	Colors = {tostring(color.Value),tostring(data.CHAR.Torso.Skill3.Value),tostring(data.CHAR.Torso.Basic.Value)}

local CONTROL = data.CHAR:WaitForChild("CharacterScript")
CONTROL:WaitForChild("LeveledUp")
local SFX = game.ServerScriptService.SFXService
local HRP = data.CHAR:WaitForChild("HumanoidRootPart")

CONTROL.LeveledUp.Event:connect(function(Level)
SFX.Artillery:Invoke(HRP.Position, 8, Colors[math.random(1, #Colors)])
data.CHAR.Effect.BrickColor = BrickColor.new(Colors[math.random(1, #Colors)])
data.CHAR.Effect2.BrickColor = BrickColor.new(Colors[math.random(1, #Colors)])
if Level % 2 == 0 then
data.CHAR.Beatbox.Transparency = 0
TRUEDJ:FireClient(PLR)
end
end)
TRUEDJ.OnServerEvent:connect(function(PLR,SongId)
local Sound = Instance.new("Sound")
Sound.Parent = data.CHAR.Beatbox
...
SFX.MusicEffects:Invoke(data.CHAR.Beatbox1, Sound,Colors[math.random(1, #Colors)],0.1)
SFX.MusicEffects:Invoke(data.CHAR.Beatbox2, Sound,Colors[math.random(1, #Colors)],0.1)
