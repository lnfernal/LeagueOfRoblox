...
function ability1(d)
	...
	d.SFX.ReverseExplosion:Invoke(center, 6,d.CHAR.Torso.Skills.Value.Color, 0.1)
	wait(0.15)		
	...
	for shot = 1, shots do
		delay(dt * shot, function()
			...
			d.DS.Damage:Invoke(enemy, damage*1, "Toughness", d.PLAYER)
				...
				d.SFX.Explosion:Invoke(center, 2, d.CHAR.Torso.Skills.Value.Color)
			end
			local function onStep(projectile)
			end
			local function onEnd(projectile)
...
function ability3(d)
	...
	d.SFX.Line:Invoke(d.CHAR.Head.Position, d.HRP.CFrame.lookVector, range, width, script.Parent.Parent.Character.Torso.Skills.Value.Color)
end
...
function ability4(d)
	...
	d.SFX.Artillery:Invoke(d.HRP.Position, d.CONTROL.GetStat:Invoke("Skillz") * 0.1, script.Parent.Parent.Character.Torso.Skills.Value.Color)
end
