...
function basicAttack(d)
	...
	d.PLAY_SOUND(d.HUMAN, 101157919, 1, 1)
	wait(0.3)
	...
	d.SFX.ProjPart:Invoke(p:ClientArgs(), part, CFrame.Angles(0, 0, 0))
...
function ability1(d)
	...
	wait(0.05)
	d.PLAY_SOUND(d.HUMAN, 101157919, 1, 2)
...
function ability2(d)
	...
	wait(0.35)
	d.PLAY_SOUND(d.HUMAN, 48577326, 5)
	...
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,d.CHAR.Torso.Skills.Value.Color, 0.25,direction,"Neon",0.039)
	for m = 1, 3 do
		d.SFX.ProjPart:Invoke(p:ClientArgs(), part, CFrame.Angles(0, math.pi*(2/3)*m, 0),
...
function ability3(d)
	...
	wait(0.1)
	d.PLAY_SOUND(d.HUMAN, 2101144, 1, 1.5)
...
local abilityData = {
	A = {
		Name = "Pentatornado",
		Desc = "Drakon Kyrane fires five tornados which deal <damage> damage apiece on contact and slow the first enemy hit by <slow>% for <duration> seconds. In symphony mode, Resyncable gains 4 volleys of shots to fire over the course of 3 seconds. However, each shot does 50% of its normal damage.",
		MaxLevel = 5,
		damage = {
			Base = 10,
			AbilityLevel = 5,
	...
	B = {
		Name = "Maelstrom Maximum",
		Desc = "Drakon Kyrane sends a insane tornado forward which detonates, dealing <damage> damage to nearby enemies and reducing their toughness and resistance by <debuff>% for <duration> seconds. In symphony mode, this move gains a larger enveloping aoe which stuns enemies for 1.25 seconds.",
		MaxLevel = 5,
		damage = {
			Base = 15,
			AbilityLevel = 5,
	...
	C = {
		...
		Desc = "Drakon Kyrane increases the movement speed of nearby allies by <bonus> and increases their attack speed by 10% for <duration> seconds. In symphony mode, the duration of the speed boost is automatically refreshed once when it runs out.",
		MaxLevel = 5,
		bonus = {
			Base = 2,
			AbilityLevel = .9,
	...
	D = {
		Name = "Dimensionia",
		Desc = "Resyncable begins to sense the rhythm of his teammates and enters symphony mode for 5 seconds. During this period, the next ability cast by Drakon Kyrane gains enhanced properties. The cooldown of this move is <cooldown> seconds.",
		MaxLevel = 3,
		cooldown = {
			Base = 55,
			AbilityLevel = -5,
