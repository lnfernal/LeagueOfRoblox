...
d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2, tostring(d.CHAR.Torso.Skills.Value) , 0.25,direction,"Neon",0.039)
end
...
function ability1(d)
	...
	d.SFX.Trail:Invoke(d.CHAR["Right Arm"], Vector3.new(0, -1, -2.5), 1, {BrickColor = d.C(tostring(d.CHAR.Torso.Skills.Value))}, 0.35, 0.8)
	wait(0.35)
	...
	local function onHit(projectile, enemy)
		projectile.Moving = false
		d.SFX.Shockwave:Invoke(d.FLAT(projectile.Position), 4, tostring(d.CHAR.Torso.Skills.Value))
...
function ability2(d)
	...
	d.SFX.Explosion:Invoke(center, range, tostring(d.CHAR.Torso.Skills.Value))
end
...
function ability4(d)
	...
	d.SFX.RisingForce:Invoke(d.HRP, Vector3.new(0, -2.5, 0), 4, 32, {BrickColor = d.C(tostring(d.CHAR.Torso.Skills.Value)), Transparency = 0.5}, 0.65)
	...
	part.BrickColor = d.CHAR.Torso.Skill3.Value
	part.Parent = game.ReplicatedStorage
	d.DB(part)
	...
	local function onEnd(projectile)
		...
		d.SFX.Shockwave:Invoke(d.FLAT(projectile.Position), 16, tostring(d.CHAR.Torso.Skills.Value))
		part:Destroy()
	end
	local p = d.DS.AddProjectile:Invoke(position, direction, speed, width, range, team, onHit, onStep, onEnd)
	d.SFX.ProjPart:Invoke(p:ClientArgs(), part, CFrame.new())
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2, tostring(d.CHAR.Torso.Skill3.Value) , 0.25,direction,"Neon",0.039)
end
