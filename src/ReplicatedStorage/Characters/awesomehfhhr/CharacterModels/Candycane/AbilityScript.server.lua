...
local colors = {"Bright red", "Lily white"}
...
function ability1(d)
	...
	d.SFX.ProjZap:Invoke(p:ClientArgs(), 1.2, colors, 0.2)
	d.SFX.ProjTrail:Invoke(p:ClientArgs(), .2,colors, 0.25,direction,"Neon",0.039) 
end
...
function ability2(d)
	...
	local t = 0
			...
			d.SFX.ReverseExplosion:Invoke(d.FLAT(center), radius, "Lily white")
...
function ability3(d)
	...
	if target then
		...
		if hrp then
			...
			d.SFX.Artillery:Invoke(hrp.Position, radius, "Bright red")
		end
	elseif target == nil then
		d.CONTROL.AbilityCooldownReduce:Invoke("C", 6.5 - (6.5 * d.CONTROL.GetStat:Invoke("CooldownReduction")))
	end
end
...
function ability4(d)
	...
	d.DS.AOE:Invoke(center, radius, team, onHit)
	d.SFX.Shockwave:Invoke(center, radius, colors)
end
