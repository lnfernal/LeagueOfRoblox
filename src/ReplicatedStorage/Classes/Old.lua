...
function Mob:GetStat(statName)
	local statBindable = self.CharacterScript:FindFirstChild("GetStat")
...
function Mob:OnDeath()
	...
	if char then
		...
		if self.Model.HumanoidRootPart.Reward.Value == "Heal" then
		self:DyingHeal(char,self.Model.HumanoidRootPart.Reward.HealAmount.Value)
		else
		self:DyingBuff(char,self.Model.HumanoidRootPart.Reward.Type.Value,self.Model.HumanoidRootPart.Reward.Amount.Value)
		self:DyingHeal(char,self.Model.HumanoidRootPart.Reward.HealAmount.Value)
		end
	end
